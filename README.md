...
OO 面向对象的编程风格在很多时候可以很大程度的简化编程模型，所以是一种非常
有用的程序组织形式，Perl6 的核心设计采用了面向对象的风格。但 Perl 6 允许多种
风格的编程模型，过程式，函数式都可以使用，这也是 Perl 文化的体现，允许百家争鸣，百花齐放。

面向对象的模块只有一个关键字，就是类的名称：

关于类的关键字：

    class has method self submethod new
...
C<$!> 定义一个私有的属性，甚至继承类也不能访问：

声明一个类方法：

    class Thingy {
        method do-something { say "Hi!" }
    }
...
Perl 5 的 Moose 模块借鉴了大量的 Perl 6 类的形式，我相信 Perl 5 终究会和 Perl 6 合并，至少可以混合编程。    

在 Windows 平台上，使用 Git shell 来进行调试，只需要输入：

    shell> cmd

在 Windows 平台上，使用 Moar 版本的发行版功能较全，我估计 Parrot 版本可能
会被废弃。因为它太大了，功能也不全。

=head2 List, Parcel and array
